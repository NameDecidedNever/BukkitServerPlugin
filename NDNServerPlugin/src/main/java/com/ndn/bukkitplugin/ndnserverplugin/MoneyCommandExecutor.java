...
public class MoneyCommandExecutor implements CommandExecutor {
	...
	if (sender instanceof Player) {
	    ...
	    if (DataManager.getInstance().makePayExchange(DataManager.getInstance().getPlayerPrimaryAccount(player.getName()), DataManager.getInstance().getPlayerPrimaryAccount(reciver), amount, reason)) {
		player.sendMessage(ChatColor.GREEN + "Transaction Complete!");
		if (plugin.getServer().getPlayer(reciver) != null) {
		    Bukkit.getPlayer(reciver).sendMessage(ChatColor.GREEN + "You've just recieved " + ChatColor.YELLOW + "$" + new java.text.DecimalFormat("0.00").format(amount) + ChatColor.GREEN + " from " + ChatColor.BLUE + player.getName() + " '" + reason + "'");
		}
	    } else {
	...
	if (sender instanceof Player) {
		...
		DataManager.getInstance().makePayExchange(DataManager.getInstance().getPlayerPrimaryAccount(args[1]), DataManager.getInstance().getPlayerPrimaryAccount(player.getName()), Double.parseDouble(args[2]), "Taken by admin");
